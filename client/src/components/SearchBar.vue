	...
	<div class="mt-16 mx-16 search-bar">
		<v-row>
			<v-col cols="9" class="cols">
				...
				<div v-if="isCSQuery || fuzzyResults.length || (getFilteredHistory.length && searchQuery)"
					...
					<div v-if="getFilteredHistory.length" class="section-container">
						...
						<div v-for="(result, index) in getFilteredHistory" :key="result.item"
							class="dropdown-item history-item" :class="{ focused: focusedIndex === index }"
							@mouseover="focusedIndex = index" @click="selectHistoryItem(result.item)">
					...
					<!-- CSQuery Section -->
					<div v-if="isCSQuery">
						<div class="dropdown-item" :class="{ focused: focusedIndex === 0 }"
							@mouseover="focusedIndex = 0">
							<div>Jira Link</div>
						...
						<div class="dropdown-item">
							...
							<div class="pill-links">
								<a v-for="(pill, index) in pillLinks" :key="pill.text" :href="pill.link" target="_blank"
									class="pill" :class="{ focused: focusedIndex === index + 2 }"
									@mouseover="focusedIndex = index + 2">{{ pill.text
									}}</a>
							</div>
						</div>
						<v-divider></v-divider>
			...
			<v-col cols="3" class="cols">
				<v-select v-model="selectedEngine" :items="searchEngines" item-title="name" item-value="url"
					variant="plain" hide-details @update:modelValue="updateSelectedEngine">
						...
						<v-list-item v-bind="props">
							<template v-slot:prepend>
								<div class="d-flex align-center">
									<v-icon v-if="item.raw.icon.startsWith('mdi-')" :icon="item.raw.icon" size="36"
										class="mr-2" />
